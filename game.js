...
const drawTextTeam = (text, x, y, color) => {
    ctx.fillStyle = color
...
const collision = (b, p) => {
    ...
    return (b.top < p.bottom && b.bottom > p.top && p.top && b.left < p.right && b.right > p.left)
}
...
const update = () => {
    ...
    if (collision(ball, player)) {
        ...
        ball.velocityY = direction * ball.speed * Math.sin(bounceAngle)
...
const render = () => {
    ...
    drawTextTeam("Görkem United", cvs.width / 6, 350, "#fff")
    drawTextTeam("Turşu City", cvs.width / 1.5, 350, "#fff")
    drawText(com.score, 2.83 * cvs.width / 4, 100, "#fff")
